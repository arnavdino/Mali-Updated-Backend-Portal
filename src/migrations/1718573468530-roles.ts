...
export class newStuff21718573468415 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `insert into role (id,name,description,permissions) values (1,"Administrator","Has access to all resources of the system",'${JSON.stringify(
        rolepermissions,
      )}')`,
    );
    await queryRunner.query(
      `insert into role (id,name,description,permissions) values (2,"Farmer","No Access",'${JSON.stringify(
        noPermissions,
      )}')`,
    );
  }
