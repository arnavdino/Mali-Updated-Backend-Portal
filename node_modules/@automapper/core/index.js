...
function map({
  ...
  const destination = mapDestinationConstructor(sourceObject, destinationIdentifier); // get extraArguments

  const extraArguments = extraArgs === null || extraArgs === void 0 ? void 0 : extraArgs(mapping, destination); // initialize an array of keys that have already been configured
...
function createInitialMapping(mapper, source, destination, configurations = []) {
  ...
  const customMappingProperties = mapping[3
  /* MappingClassId.customProperties */
  ];
  const hasCustomMappingProperties = customMappingProperties.length > 0;
  const namingConventions = mapping[8
  /* MappingClassId.namingConventions */
  ];
  const metadataMap = getMetadataMap(mapper);
  ...
  for (let i = 0, length = destinationPaths.length; i < length; i++) {
    ...
    if (hasCustomMappingProperties && customMappingProperties.some(property => isPrimitiveArrayEqual(property[0
    ...
    if (nestedMappingPair) {
      ...
      const mappingTypeConverters = mapping[6
...
function typeConverter(source, destination, converterOrValueSelector) {
  return mapping => {
    ...
    const typeConverters = mapping[6
    /* MappingClassId.typeConverters */
    ] || (mapping[6
    /* MappingClassId.typeConverters */
    ] = new Map());
