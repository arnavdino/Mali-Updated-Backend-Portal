...
class ModelVisitor {
  ...
  static visit(sourceFile, context, program) {
    ...
    function nodeVisitorFactory(ctx, sf) {
      const nodeVisitor = node => {
        ...
        if (tsserverlibrary.isPropertyDeclaration(node) || tsserverlibrary.isGetAccessorDeclaration(node)) {
          ...
          const isPropertyStaticOrPrivate = (node.modifiers || []).some(modifier => modifier.kind === tsserverlibrary.SyntaxKind.StaticKeyword || modifier.kind === tsserverlibrary.SyntaxKind.PrivateKeyword); // if this property is static or private, skip because
    ...
    const visitedSourceFile = tsserverlibrary.visitNode(sourceFile, nodeVisitorFactory(context, sourceFile)); // if the target is CommonJS, keep as is
